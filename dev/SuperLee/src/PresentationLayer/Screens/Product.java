...
public class Product extends Screen{
    ...
    public Product(Screen caller, Domain.ServiceLayer.Objects.Product sProduct, String[] extraMenuOptions) {
        super(caller, Stream.concat(Arrays.stream(menuOptions), Arrays.stream(extraMenuOptions)).toArray(String[]::new), "Product "+sProduct.getName());
...
//                printUpcomingShifts();
    ...
    public void changeProductMin() {
        ...
        Result<Domain.ServiceLayer.Objects.Product> r = controller.changeProductMin(store, product, min);
        if (r.isError()) {
    ...
    public void changeProductMax() {
        ...
        scanner.nextLine(); //to remove extra \n
        Result<Domain.ServiceLayer.Objects.Product> r = controller.changeProductMax(store, product, max);
        if (r.isError()) {
            System.out.println(r.getError());
        }
        else {
...
//    }
    ...
    public void addProductToStore() {
        ...
        int max = scanner.nextInt();
        scanner.nextLine(); //to remove extra \n
        Result<Domain.ServiceLayer.Objects.Product> r = controller.addProductToStore(store, inStore, inWareHouse, product, min, max);
        if (r.isError())
            System.out.println(r.getError());
        else {
    ...
    public void removeProductFromStore() {
        ...
        int product = scanner.nextInt();
        scanner.nextLine(); //to remove extra \n
        Result<Domain.ServiceLayer.Objects.Product> r = controller.removeProductFromStore(store, product);
        if (r.isError())
            System.out.println(r.getError());
        else {
    ...
    public void changeProductCategory() {
        ...
        scanner.nextLine(); //to remove extra \n
        Result<Domain.ServiceLayer.Objects.Product> r = controller.moveProductToCategory(productID, category);
        if (r.isError())
            System.out.println(r.getError());
        else {
            System.out.println("product successfully moved to new category");
            Domain.ServiceLayer.Objects.Product p = r.getValue();
    ...
    public void changeProductPrice() {
        ...
        scanner.nextLine(); //without this line the next scanner will be passed without the user's input.
        Result<Domain.ServiceLayer.Objects.Product> r = controller.editProductPrice(id, price);
        if (r.isError())
            System.out.println(r.getError());
        else {
            Domain.ServiceLayer.Objects.Product p = r.getValue();
    ...
    public void changeProductName() {
        ...
        Result<Domain.ServiceLayer.Objects.Product> r = controller.editProductName(id, name);
        if (r.isError())
            System.out.println(r.getError());
        else {
            Domain.ServiceLayer.Objects.Product p = r.getValue();
    ...
    public void addProduct() {
        ...
        Result<Domain.ServiceLayer.Objects.Product> r = controller.newProduct(info[0],Integer.parseInt(info[1]), Integer.parseInt(info[2]), Double.parseDouble(info[3]), suppliersMap, Integer.parseInt(info[5]));
        if (r.isError())
            System.out.println(r.getError());
        else {
            Domain.ServiceLayer.Objects.Product p = r.getValue();
