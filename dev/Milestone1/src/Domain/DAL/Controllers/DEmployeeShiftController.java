    ...
    public Set<Pair<Pair<Date, ShiftTypes>, Set<String>>> loadData() {
        try {
            ...
            for (int i=0;i<40;i=i+5)
                employees.add(""+i);
            employees.add(""+8);
            shiftEmployee.add(new Pair<>(new Pair<>(new SimpleDateFormat("dd-MM-yyyy").parse("19-06-2021"),ShiftTypes.Evening),employees));
            employees = new HashSet<>(employees);
            employees.add("2");
            shiftEmployee.add(new Pair<>(new Pair<>(new SimpleDateFormat("dd-MM-yyyy").parse("23-07-2021"),ShiftTypes.Evening),employees));
            employees = new HashSet<>(employees);
            employees.add("6");
            shiftEmployee.add(new Pair<>(new Pair<>(new SimpleDateFormat("dd-MM-yyyy").parse("15-06-2021"),ShiftTypes.Morning),employees));
            employees = new HashSet<>(employees);
            employees.add("4");
            shiftEmployee.add(new Pair<>(new Pair<>(new SimpleDateFormat("dd-MM-yyyy").parse("18-06-2021"),ShiftTypes.Morning),employees));
